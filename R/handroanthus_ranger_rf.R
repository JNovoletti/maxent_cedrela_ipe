# =========================================================================
# SDM Handroanthus (ipê) – Random Forest (ranger) + VSURF
# Área de estudo  : Amazônia Legal
# Fontes de dados : GBIF Brasil (data/handroanthus_var_amb.csv)
...
out_dir <- here::here("output_ranger", "handroanthus")
...
# raster_files <- c("C:/Users/Aluno/Documents/dslab/João/maxent_cedrela_ipe/rasters/32.vapor.tif","C:/Users/Aluno/Documents/dslab/João/maxent_cedrela_ipe/rasters/33.solar.tif")
...
gbif_br <- tryCatch(
  read.csv(here::here("data", "handroanthus_var_amb.csv")),
  error = function(e) {
    message("handroanthus_var_amb.csv não encontrado; usando handroanthus_br.csv")
    read.csv(here::here("data", "handroanthus_br.csv"))
  }
)
...
if (!is.null(amostras_raw)) {
  amostras_handro <- amostras_raw %>%
    dplyr::filter(Genus == "Handroanthus") %>%
    dplyr::mutate(
  ...
  amostras_handro <- data.frame(lon = numeric(0), lat = numeric(0), fonte = character(0))
}

# --- 4.3 Combinar, remover NA e duplicatas -------------------------------

occ_all <- dplyr::bind_rows(gbif_pts, amostras_handro) %>%
  dplyr::filter(
    !is.na(lon), !is.na(lat),
    lon >= -180, lon <= 180,
    lat >= -90,  lat <= 90
...
occ_thin_in <- occ_pts %>%
  dplyr::select(lon, lat) %>%
  dplyr::mutate(especie = "Handroanthus")
...
cat("Handroanthus – pontos após thinning de 5 km:", nrow(occ_final), "\n")
...
gg_imp <- ggplot(imp_df, aes(x = reorder(variavel, importancia), y = importancia)) +
  ...
  coord_flip() +
  labs(
    title = expression(italic("Handroanthus") ~ "– Importância das variáveis (RF)"),
    x     = "Variável",
...
ggsave(
  file.path(here::here("figuras"), "handroanthus_rf_importancia.png"),
...
mapa_gg <- ggplot() +
  ...
  labs(
    title = expression(italic("Handroanthus") ~ "– probabilidade de ocorrência (RF)"),
...
ggsave(
  file.path(here::here("figuras"), "handroanthus_probabilidade_ocorrencia.png"),
...
mapa_bin_gg <- ggplot() +
  ...
  scale_fill_manual(
    name   = "Classe",
    values = c("Ausente" = "#d9d9d9", "Presente" = "#006837"),
    drop   = FALSE, na.value = "transparent"
  ) +
  labs(
    title = paste0("Handroanthus – presença binária (maxSSS = ", round(limiar_maxSSS, 3), ")"),
    x = "Longitude", y = "Latitude"
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
ggsave(
  file.path(here::here("figuras"), "handroanthus_binario_rf.png"),
...
terra::writeRaster(
  prob_map,
  file.path(out_dir, "handroanthus_prob_ocorrencia.tif"),
  overwrite = TRUE
)

terra::writeRaster(
  bin_map,
  file.path(out_dir, "handroanthus_binario_rf.tif"),
...
writexl::write_xlsx(
  ...
  path = file.path(out_dir, "handroanthus_rf_resultados.xlsx")
)

saveRDS(rf_final,  file.path(out_dir, "modelo_handroanthus_ranger.rds"))
saveRDS(vsurf_res, file.path(out_dir, "vsurf_handroanthus.rds"))

save.image(file.path(out_dir, "handroanthus_ranger_workspace.RData"))

cat("\n=== Handroanthus RF concluído ===\n")
